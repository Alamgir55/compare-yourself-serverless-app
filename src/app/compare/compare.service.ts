...
export class CompareService {
  ...
  onStoreData(data: CompareData) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session) => {
      ...
      this.http.post('https://asuef2kc4f.execute-api.us-east-1.amazonaws.com/dev/compare-yourself', data, {
  ...
  onRetrieveData(all = true) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session) => {
      ...
      this.http.get('https://asuef2kc4f.execute-api.us-east-1.amazonaws.com/dev/compare-yourself/' + urlParam + queryParam, {
        headers: new Headers({'Authorization': session.getIdToken().getJwtToken()})
      })
        .map(
